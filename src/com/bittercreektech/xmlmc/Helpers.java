...
public class Helpers {
     ...
     */
    public static Map<String, String> nodesAsMap(NodeList nodes) {
        Map<String, String> nodeMap = new HashMap<String, String>();
        int nodeIndex = 0;
        while (nodeIndex < nodes.getLength()) {
            Node node = nodes.item(nodeIndex);
            if (!node.getNodeName().equals("#text")) {
                nodeMap.put(node.getNodeName(), node.getTextContent().toLowerCase());
            }
            nodeIndex++;
        }
    ...
    public static Map<String, String> nodesAsMap(Node node) {
