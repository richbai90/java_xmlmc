...
import com.bittercreektech.xmlmc.*;
...
public class Call implements SwType {
     ...
     */
    public void setCustomer(String customer) throws XmlmcException {
        Data data = xmlmc.data();
        ...
        try {
            ...
            try {
                ...
                throw new XmlmcException(String.format("%s (%s)", ErrorCodes.RECORD_NOT_FOUND.toString(), customer), exc, xmlmc);
            }
...
//            organisation = orgRecord.getRow(0).get("companyname");
    ...
    /**
     * This method is used by the {@link Helpdesk} API when assigning calls to groups. It does not need to be called
    ...
    /**
     * Analyst to assign the call to. Is used by the {@link Helpdesk} API when logging and assigning calls
     * And could cause errors when called manually.
     *
     * @param group   Group to assign the call to
     * @param analyst Analyst to assign the call to
     * @see Helpdesk#logAndAssignNewCall(Call, String, String)
